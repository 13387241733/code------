...
#define LEVEL6_TIMER 30    // 难度6收缩周期改为30秒
...
// 绘制边界（确保边界始终可见）
void drawBorder(int left, int top, int right, int bottom, char fill_char) {
    // 保存当前颜色
...
// 检查位置是否在边界上
int isOnBorder(int x, int y, int left, int top, int right, int bottom) {
    return (x == left || x == right || y == top || y == bottom);
}

// 检查位置是否在有效游戏区域内（不包括边界）
int isInPlayArea(int x, int y, int left, int top, int right, int bottom) {
    return (x > left && x < right && y > top && y < bottom);
}
...
int main()
{
    ...
    // 用黄色显示游戏场地建议，醒目但不刺眼
    setColor(14);
    printf("游戏场地大小建议：窗口全屏游玩，用150*40\n");
    setColor(7); // 恢复默认白色
    printf("0级：基础模式\n");
    ...
    printf("6级：收缩边界模式\n");
    printf("   - 强制场地初始为150*40\n");
    printf("   - 包含1条玩家蛇和2条敌对蛇\n");
    printf("   - 每30秒边界向内收缩1格\n");  // 更新为30秒
    printf("   - 直至场地无法容纳蛇活动，生存到最后的获胜\n\n");
    ...
    // 计算场地参数
    int fieldArea = (borderRight - borderLeft - 1) * (borderBottom - borderTop - 1);
    int baseSpeed = 100;
    ...
    for (int i = 0; i < player.length; i++) {
        player.x[i] = (borderLeft + borderRight) / 2 - i;
        player.y[i] = (borderTop + borderBottom) / 2;
    }
    ...
    else if (difficulty == 6) enemyCount = 2;
    ...
    for (int i = 0; i < enemyCount; i++) {
        ...
        // 初始位置在游戏区域内且错开
        enemySnakes[i].x[0] = borderLeft + 5 + (i * 10);
        enemySnakes[i].y[0] = borderTop + 5 + (i * 5);
    ...
    system("cls");
    // 绘制初始边界
    drawBorder(borderLeft, borderTop, borderRight, borderBottom, fill_char);
    ...
    while (currentFoodCount < targetFoodCount && currentFoodCount < MAX_FOOD) {
        ...
        if (!foods[idx].active) {
            ...
            do {
                valid = 1;
                // 确保食物生成在游戏区域内（不包括边界）
                foods[idx].x = borderLeft + 1 + rand() % (borderRight - borderLeft - 1);
                foods[idx].y = borderTop + 1 + rand() % (borderBottom - borderTop - 1);
    ...
    DWORD level6LastShrink = GetTickCount();  // 难度6边界收缩计时
    ...
    while (!gameOver) {
        ...
        // 难度6：边界收缩逻辑（已改为30秒）
        if (difficulty == 6) {
            if (currentTime - level6LastShrink >= LEVEL6_TIMER * 1000) {
                // 边界向内收缩1格
                borderLeft++;
                borderTop++;
                borderRight--;
                ...
                // 检查场地是否还能容纳蛇
                if (borderRight - borderLeft < 10 || borderBottom - borderTop < 10) {
                    // 场地过小，判断胜负
                    gameOver = 1;
                    int playerAlive = !player.respawning && player.lives > 0;
                    int enemiesAlive = 0;
                    for (int k = 0; k < enemyCount; k++) {
                        if (!enemySnakes[k].respawning && enemySnakes[k].lives > 0) {
                            enemiesAlive++;
                        }
                    }
                    ...
                    if (playerAlive && enemiesAlive == 0) {
                        setColor(10);
                        printf("恭喜获胜！你是最后的幸存者！\n");
                    }
                    else if (!playerAlive && enemiesAlive > 0) {
                        setColor(12);
                        printf("游戏结束！敌人存活到了最后！\n");
                    }
                    else {
                        setColor(14);
                        printf("游戏结束！场地已无法容纳任何蛇活动！\n");
                    }
                    setColor(7);
                    printf("你的分数: %d\n", player.score);
                    for (int k = 0; k < enemyCount; k++) {
                ...
                for (int x = 0; x < W; x++) {
                    for (int y = 0; y < H; y++) {
                        if (!isInPlayArea(x, y, borderLeft, borderTop, borderRight, borderBottom) &&
                            !isOnBorder(x, y, borderLeft, borderTop, borderRight, borderBottom)) {
                            gotoxy(x, y);
                            printf(" ");
                        }
                    }
                ...
                for (int i = 0; i < MAX_FOOD; i++) {
                    if (foods[i].active && !isInPlayArea(foods[i].x, foods[i].y, borderLeft, borderTop, borderRight, borderBottom)) {
                        foods[i].active = 0;
                        currentFoodCount--;
                    }
                }

                level6LastShrink = currentTime;

                // 显示收缩提示
                gotoxy((borderLeft + borderRight) / 2 - 10, borderTop + 1);
                setColor(12);
                printf("边界正在收缩！");
                Sleep(500);
                gotoxy((borderLeft + borderRight) / 2 - 10, borderTop + 1);
        ...
        // 清除玩家蛇旧位置（只清除游戏区域内的部分）
        for (int i = 0; i < player.length; i++) {
            if (isInPlayArea(player.x[i], player.y[i], borderLeft, borderTop, borderRight, borderBottom)) {
                gotoxy(player.x[i], player.y[i]);
                printf(" ");
            }
        }
        ...
        if (player.respawning) {
            if (currentTime >= player.respawnEndTime) {
                player.respawning = 0;
                // 重生位置初始化（确保在边界内）
                player.x[0] = (borderLeft + borderRight) / 2;
                player.y[0] = (borderTop + borderBottom) / 2;
                player.length = 3;
        ...
        int collision = 0;
        // 撞墙检测（即使无敌也不能穿墙）
        if (player.x[0] <= borderLeft || player.x[0] >= borderRight ||
            player.y[0] <= borderTop || player.y[0] >= borderBottom) {
            collision = 1;
        }
        ...
        if (collision) {
            // 重绘被蛇头覆盖的边界部分
            if (isOnBorder(player.x[0], player.y[0], borderLeft, borderTop, borderRight, borderBottom)) {
                gotoxy(player.x[0], player.y[0]);
                setColor(15);
                printf("%c", fill_char);
            }
        ...
        for (int k = 0; k < enemyCount; k++) {
            if (enemySnakes[k].respawning) {
                if (currentTime >= enemySnakes[k].respawnEndTime) {
                    ...
                    // 敌人重生（在当前边界内）
                    enemySnakes[k].x[0] = borderLeft + 5 + rand() % (borderRight - borderLeft - 10);
                    enemySnakes[k].y[0] = borderTop + 5 + rand() % (borderBottom - borderTop - 10);
            ...
            // 清除敌人蛇旧位置（只清除游戏区域内的部分）
            for (int i = 0; i < enemySnakes[k].length; i++) {
                if (isInPlayArea(enemySnakes[k].x[i], enemySnakes[k].y[i], borderLeft, borderTop, borderRight, borderBottom)) {
                    gotoxy(enemySnakes[k].x[i], enemySnakes[k].y[i]);
                    printf(" ");
                }
            }
            ...
            int attackPlayer = 0;
            if (playerDist < 15 && !player.invincible && !player.respawning) {
            ...
            // 边界碰撞处理（防止穿墙）
            if (enemySnakes[k].x[0] <= borderLeft + 1) {
                enemySnakes[k].x[0] = borderLeft + 1;
                if (enemySnakes[k].direction == LEFT)
                    enemySnakes[k].direction = rand() % 2 ? UP : DOWN;
            }
            if (enemySnakes[k].x[0] >= borderRight - 1) {
                enemySnakes[k].x[0] = borderRight - 1;
                if (enemySnakes[k].direction == RIGHT)
                    enemySnakes[k].direction = rand() % 2 ? UP : DOWN;
            }
            if (enemySnakes[k].y[0] <= borderTop + 1) {
                enemySnakes[k].y[0] = borderTop + 1;
                if (enemySnakes[k].direction == UP)
                    enemySnakes[k].direction = rand() % 2 ? LEFT : RIGHT;
            }
            if (enemySnakes[k].y[0] >= borderBottom - 1) {
                enemySnakes[k].y[0] = borderBottom - 1;
            ...
            // 撞墙检测
            if (enemySnakes[k].x[0] <= borderLeft || enemySnakes[k].x[0] >= borderRight ||
                enemySnakes[k].y[0] <= borderTop || enemySnakes[k].y[0] >= borderBottom) {
                enemyCollision = 1;

                // 重绘被敌人蛇头覆盖的边界部分
                gotoxy(enemySnakes[k].x[0], enemySnakes[k].y[0]);
                setColor(15);
                printf("%c", fill_char);
            }
    ...
    // 游戏结束处理
    if (gameOver && difficulty != 5 && difficulty != 6) {
        system("cls");
        setColor(12);
        printf("游戏结束！\n");
        setColor(7);
